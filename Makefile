postgres:
	docker run --name postgres12 --network bank-network -p 5432:5432 -e POSTGRESS_USER=root -e POSTGRES_PASSWORD=secret -d postgres:12-alpine
...
mock: 
	mockgen -package mockdb -destination db/mock/store.go github.com/justIGreK/simple_bank_course/db/sqlc Store
